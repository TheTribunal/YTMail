...
def create_ticket(conn, ticket):
    ...
    """
    # TODO: add timestamps
    sql = """ INSERT INTO tickets (discord_user, status) VALUES (?, ?) """
    cur = conn.cursor()
...
def create_msg_log(conn, msg_log):
    ...
    """
    # TODO: add timestamps
    sql = """ INSERT INTO msg_logs (ticket_id, sender, msg_content, bot_msg_id) VALUES (?, ?, ?, ?) """
    cur = conn.cursor()
...
def update_ticket_status(conn, ticket_id):
    sql = """ UPDATE tickets
     SET status = ? 
     WHERE id = ? """
    cur = conn.cursor()
...
def main():
    ...
    with conn:
        ...
        msg_log = (ticket_id, user, 'help! little timmy fell down a well!', 691693885465952347)
